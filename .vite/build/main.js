...
electron.app.commandLine.appendSwitch("enable-features", "Vulkan,VaapiVideoDecoder,CanvasOopRasterization");
if (started) {
  electron.app.quit();
}
...
electron.app.whenReady().then(() => {
  electron.protocol.handle("quake-file", (request) => {
    var _a;
    const url = request.url.replace("quake-file://", "");
    const quakePath = (_a = store.get("bakamula")) == null ? void 0 : _a.quakePath;
    if (!quakePath) {
      return new Response("Quake path not configured", { status: 404 });
    }
    const filePath = path$1.join(quakePath, decodeURIComponent(url));
    if (!filePath.startsWith(path$1.resolve(quakePath))) {
...
electron.ipcMain.handle("config:get", (_, key) => store.get(key));
electron.ipcMain.handle("config:set", (_, key, value) => store.set(key, value));
electron.ipcMain.handle("gpu:getStatus", () => electron.app.getGPUFeatureStatus());
...
electron.ipcMain.handle("fs:listSubfolders", (_, dirPath) => {
  try {
    const entries = fs.readdirSync(dirPath);
    return entries.filter((entry) => {
      try {
        return fs.statSync(path$1.join(dirPath, entry)).isDirectory();
      } catch {
        return false;
      }
    });
...
electron.ipcMain.handle("fs:readFile", (_, filePath) => {
  try {
    ...
    const resourcePath = path$1.join(process.resourcesPath, filePath);
    if (existsSync(resourcePath)) {
      return readFileSync(resourcePath, "utf8");
    }
    const rootPath = path$1.join(electron.app.getAppPath(), filePath);
